...
func (w *RedisWorker) processMessage(ctx context.Context, stream string, group string, message redis.XMessage) {
	...
	// Reconstruct the originating distributed trace from the envelope.
	// This creates a LINKED span — the worker span is a child of the
	// HTTP handler span that enqueued the job, bridging the async boundary.
	if envelope.TraceParent != "" {
		carrier := propagation.MapCarrier{
			"traceparent": envelope.TraceParent,
			"tracestate":  envelope.TraceState,
		}
		propCtx := propagation.TraceContext{}.Extract(jobCtx, carrier)
		// Start a linked span so the job execution appears under the original trace.
		tracer := otel.Tracer("astra/queue")
		propCtx, span := tracer.Start(propCtx, "job/"+envelope.JobType,
	...
	if w.dashboard != nil {
		w.dashboard.TrackJob(envelope.JobType, "started", map[string]any{
			"job_id": envelope.ID,
			"queue":  envelope.Queue,
		}, 0)
	}
	...
	if runErr == nil {
		...
		if w.dashboard != nil {
			w.dashboard.TrackJob(envelope.JobType, "completed", map[string]any{
				"job_id": envelope.ID,
				"queue":  envelope.Queue,
			}, duration)
		}
	...
	if w.dashboard != nil {
		w.dashboard.TrackJob(envelope.JobType, "failed", map[string]any{
			"job_id": envelope.ID,
			"queue":  envelope.Queue,
			"error":  runErr.Error(),
		}, duration)
