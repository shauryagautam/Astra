...
func ScanIter[T any](db *DB, rows Rows, meta *ModelMeta) iter.Seq2[*T, error] {
	return func(yield func(*T, error) bool) {
		...
		for rows.Next() {
			...
			if err := db.scanRow(rows, columns, colMetas, colValid, v); err != nil {
				if !yield(nil, err) {
					return
				}
				continue
...
// Deprecated: Use ScanIter for more efficient processing.
func (db *DB) scanRows(rows Rows, meta *ModelMeta) (any, error) {
	defer rows.Close()

	// We still use reflect to build the slice because meta.Type is dynamic at this level.
	// However, we now use the logic shared with ScanIter by calling it if we knew the type,
	// but here we just implement the loop for the reflect-based slice.
