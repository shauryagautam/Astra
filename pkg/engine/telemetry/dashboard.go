...
// Subscribe returns a channel that receives new dashboard entries.
...
func (d *Dashboard) Subscribe() (<-chan DashboardEntry, func()) {
	...
	// Use a slightly larger buffer to handle bursty telemetry
	ch := make(chan DashboardEntry, 100)
...
func (d *Dashboard) addEntry(entry DashboardEntry) {
	...
	for _, sub := range d.subscribers {
		...
		case sub.ch <- entry:
		default:
			// Buffer full, skip this subscriber to avoid framework stalling
		}
	}
}
